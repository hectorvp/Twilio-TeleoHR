...
app.post('/exit/', (req, res) => {
	...
	app.post('/question/',(req,res)=>{
		...
		if( parseInt(req.query.questionIndex) != 0 )   
			response.say('OK, your answer has been recorded');
	...
	app.get('/welcome',(req,res) => {
		...
		if( !req.query.candidateIndex || req.query.candidateIndex == 0 )
			candidateIndex = 0;
		else
			candidateIndex = req.query.candidateIndex;
		
		let url = "http://18.212.20.107:8855/question/?questionIndex=0&candidateIndex="+candidateIndex;
		
		let gather = localResponse.gather({input:'speech' ,action: url, method: 'POST', hints: 'yes,okay,sure,for sure,go ahead', language:'en-IN', speechTimeout:'auto', enhanced:'true'});
		gather.say('Hi, This is HR assistant from T-systems, we have shortlisted your profile for Java developer role & need few details of you. Should we start with the set of questions ?');
      ...
      .then(call => res.send(call.sid),error => res.send(error));
